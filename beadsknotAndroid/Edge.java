...
class Edge {
    ...
    void connect_nodes(Paint p, Canvas c, ArrayList<Node> nodes, double l, double t, double r, double b){
        Path path = new Path();
        // スタート地点を移動
        //Log.d("hとjを表示",""+h+"  "+j);
        double wid = r-l;
        ...
        path.moveTo((float)(hx), (float)(hy));
        // 制御点1 X, 制御点1 Y, 制御点2 X, 制御点2Y, 終点X, 終点Y
        path.cubicTo((float)(ix), (float)(iy),
                (float)(kx), (float)(ky),
                (float)(jx), (float)(jy));
        p.setARGB(255, 0, 0, 0);
    ...
    private static double naibun(double p, double q, double t) {
        return (p*(1.0-t)+q*t);
    }

    private static double coordinate_bezier(double a, double c, double e, double g, double t) {
    ...
    private static double get_rangewidth_angle(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4) {
        double ret0 = (Math.PI);
        double ret1 = 0;
    ...
    static void rotation_shape_modifier(ArrayList<Node> nodes, ArrayList<Edge> edges) {//円を自動で回転させる
    ...
    void setDrawOn(ArrayList<Node> nodes, ArrayList<Edge> edges){
        ...
        do{
            ...
            Log.d("getArea", cursorEdge.getName());
